...
:config
(winner-mode t)
(define-key global-map (kbd "<right>") 'winner-redo)
(define-key global-map (kbd "<left>")  'winner-undo))
#+END_SRC
...
#+BEGIN_SRC emacs-lisp :tangle yes
  (req-package tern
    ...
    (add-hook 'tern-mode-hook
              #'(lambda ()
                  (define-key tern-mode-keymap (kbd "C-c") nil)
                  (define-key tern-mode-keymap (kbd "C-d") nil)
                  (define-key tern-mode-keymap (kbd "C-r") nil)
                  (evil-leader/set-key
