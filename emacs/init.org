...
#+BEGIN_SRC emacs-lisp :tangle yes
  ...
  (defun rcoedo/layout/99 ()
    ...
    (rcoedo/window/split-horizontally-focus-next-buffer)
    (other-window 1)
    (rcoedo/window/split-horizontally-next-buffer))

  (defun rcoedo/layout/00 ()
    (interactive)
    (delete-other-windows)
    (rcoedo/window/split-horizontally-next-buffer))

  ;; Not working as intended with local variables.
  (defun rcoedo/layout/next ()
    (interactive)
    (let ((layout (pop rcoedo/layout/layout-list)))
      (nreverse (cons layout (nreverse rcoedo/layout/layout-list)))
      (funcall layout)))
#+END_SRC
...
#+BEGIN_SRC emacs-lisp :tangle yes
  (req-package evil-leader
    ...
    (defun rcoedo/evil-leader/set-mode-key (element)
      (evil-leader/set-key (car element) (car (cdr element))))

    (defun rcoedo/evil-leader/set-mode-keys (bindings)
      (let ((prefixed-bindings (-map 'rcoedo/evil-leader/prefix-mode-binding bindings)))
        (-map 'rcoedo/evil-leader/set-mode-key prefixed-bindings))))
#+END_SRC
...
#+BEGIN_SRC emacs-lisp :tangle yes
  (req-package projectile-rails
    ...
    (progn
      ...
      (defun rcoedo/rails/layout-99 ()
        (rcoedo/layout/99)
        (other-window 1)
        (other-window 1)
        (if (not (eq nil (get-buffer "*rails*"))) (switch-to-buffer "*rails*"))
        (other-window 1)
        (other-window 1))

      (defun rcoedo/rails/hook ()
        (setq-local rcoedo/layout/layout-list '(rcoedo/layout/00 rcoedo/layout/08 rcoedo/rails/layout-99)))

      (add-hook 'projectile-rails-mode-hook 'rcoedo/rails/hook)))
#+END_SRC
...
#+BEGIN_SRC emacs-lisp :tangle yes
  (req-package tern
    ...
    (progn
      ...
      (add-hook 'tern-mode-hook 'rcoedo/tern/bindings)))
#+END_SRC
...
#+BEGIN_SRC emacs-lisp :tangle yes
  ...
  (setq-default c-basic-offset 4
                ...
                rcoedo/layout/layout-list '(rcoedo/layout/00 rcoedo/layout/08 rcoedo/layout/99))
  ...
  (define-key global-map (kbd "M-n")           'rcoedo/buffer/next-non-emacs-buffer)
  (define-key global-map (kbd "M-p")           'rcoedo/buffer/previous-non-emacs-buffer)
  (define-key global-map (kbd "M-f")           'delete-other-windows)
  (define-key global-map (kbd "M-e")           'rcoedo/eshell/projectile-eshell-popup)
  (define-key global-map (kbd "C-l")           'rcoedo/layout/next)
